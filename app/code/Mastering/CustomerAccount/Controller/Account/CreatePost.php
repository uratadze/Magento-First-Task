...
namespace Mastering\CustomerAccount\Controller\Account;
...
class CreatePost extends AbstractAccount implements CsrfAwareActionInterface, HttpPostActionInterface
{
     ...
     * @return bool
     */
    public function customValidate($request): bool
    {
        $personNumber = $request->getParam('person_number');
        $isNumber = is_numeric($personNumber);
        $isNumber ? '':$this->_messageManager->addError('Please enter number in Person Number');
        $numberLen = strlen((string)$personNumber);
        $personNumberCheck = !($numberLen < 9 || $numberLen > 11);
        if(!empty($personNumber) && $isNumber)
        {
            $personNumberCheck ? '':$this->_messageManager->addError('Please enter number in range 9, 11');
        }
        return $this->formKeyValidator->validate($request) && ($isNumber && $personNumberCheck);
    }
    ...
    public function execute()
    {
        ...
        if (!$this->getRequest()->isPost()
            || !$this->customValidate($this->getRequest())
        ) {
            $url = $this->urlModel->getUrl('*/*/create', ['_secure' => true]);
        ...
        try {
            ...
            $personNumber = $this->getRequest()->getParam('person_number');
            $customer->setCustomAttribute('person_number', $personNumber);
